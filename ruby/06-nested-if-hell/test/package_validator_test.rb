...
require_relative '../src/package_validator'

class PackageValidatorTest < Minitest::Test
  def setup
  ...
  def test_rejects_extreme_temperature
    result = @validator.can_deliver(weight: 10, hazardous: false, weekend: false, temperature_required: 50)
    assert_equal false, result[:allowed]
    assert_equal 'Temperature out of range', result[:warning]
  end

  def test_allows_valid_temperature
    result = @validator.can_deliver(weight: 10, hazardous: false, weekend: false, temperature_required: 20)
    assert_equal true, result[:allowed]
  end

  def test_allows_remote_small_package
    result = @validator.can_deliver(weight: 15, hazardous: false, weekend: false, remote_area: true)
    assert_equal true, result[:allowed]
    assert_equal 'Remote surcharge applies', result[:warning]
  end

  def test_rejects_remote_heavy_package
    result = @validator.can_deliver(weight: 25, hazardous: false, weekend: false, remote_area: true)
    assert_equal false, result[:allowed]
    assert_equal 'Too heavy for remote', result[:warning]
  end

  def test_rejects_null_package
    result = @validator.can_deliver(nil)
    assert_equal false, result[:allowed]
  ...
  def test_temperature_40_allowed_at_boundary
    result = @validator.can_deliver(weight: 10, hazardous: false, weekend: false, temperature_required: 40)
    assert_equal true, result[:allowed]
  end
  ...
  def test_remote_weight_20_allowed_at_boundary
    result = @validator.can_deliver(weight: 20, hazardous: false, weekend: false, remote_area: true)
    assert_equal true, result[:allowed]
    assert_equal 'Remote surcharge applies', result[:warning]
  end
end
